...
import cn.cuilan.annotation.RedisCached;
import cn.cuilan.common.BaseIdEntity;
import cn.cuilan.common.BaseIdTimeEntity;
import cn.cuilan.mapper.CachedMapper;
import cn.cuilan.redis.EntityRedisPrefix;
import cn.cuilan.redis.RedisUtils;
...
public class MapperAspect {
     ...
     */
    @Around("execution(* cn.cuilan.mapper.*Mapper.insert(..))")
     ...
     * 并设置更新时间。
     */
    @Around("execution(* cn.cuilan.mapper.*Mapper.update*(..))")
     ...
     */
    @Around("execution(* cn.cuilan.mapper.*Mapper.*(..))")
    ...
    @Around("execution(* cn.cuilan.mapper.*Mapper.selectCacheById(..))")
    ...
    @Around("execution(* cn.cuilan.mapper..*Mapper.selectCacheByIds(..))")
     ...
     * 增强 Mapper，提供批量插入的能力
     */
    @Around("execution(* cn.cuilan.mapper.CommonMapper.saveBatch(..))")
    public Object saveBatch(ProceedingJoinPoint pjp) {
        return batchOperation(pjp);
    }

    // 批量更新
    @Around("execution(* cn.cuilan.mapper.CommonMapper.updateBatchById(..))")
    public Object updateBatchById(ProceedingJoinPoint pjp) {
        return batchOperation(pjp);
    }

    // 批量更新或插入
    @Around("execution(* cn.cuilan.mapper.CommonMapper.saveOrUpdateBatch(..))")
